...
class ProfileTests(unittest.TestCase):
    ...
    def test_motion_stops_for_starvation_story(self):
        ...
        self.assertIsNone(root.find(f'.//{{{NS}}}animateMotion'))
        css=root.find(f'{{{NS}}}style').text
        self.assertIn('@keyframes move0',css)
        self.assertIn('linear infinite',css)
        positions=re.findall(r'translate\((-?[\d.]+)px,(-?[\d.]+)px\)',css)
    ...
    def test_snake_eats_first_and_stops_before_second(self):
        ...
        result=smooth(svg)
        root=ET.fromstring(result)
        self.assertEqual(root.findall(f'.//{{{NS}}}animateMotion'),[])
        css=root.find(f'{{{NS}}}style').text
        cycle=float(re.search(r'animation:move0 ([\d.]+)s',css)[1])
        self.assertLess(cycle,4)
        motion0=re.search(r'@keyframes move0\{((?:[^{}]*\{[^{}]*\})+)\}',css)[1]
        self.assertIn('100%{transform:translate(64px,24px)}',motion0)
        end_percent=float(re.search(r'([\d.]+)%,100%',motion0)[1])
        self.assertAlmostEqual(104/(end_percent/100*cycle),240,places=3)
        ...
        self.assertIsNotNone(root.find(f'.//{{{NS}}}g[@class="snake-motion segment-0"]/{{{NS}}}path[@class="dead-eyes"]'))
        ...
        self.assertIn('100%{transform:translate(64px,8px)}',css)
