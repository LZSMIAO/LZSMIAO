...
完整週報預設折疊，展示每日時長、四種主要語言與「其他」、語言時長及佔比。AI 區域僅彙總 grand_total 中的輸入/輸出 Token、AI 新增行、提問次數和 ai_model_total_cost；金額為 API 價格估算，不是實際帳單。任何 AI 欄位缺少任一天的數據時顯示「—」，不把缺失值當成零。未取得 AI 數據時省略該區域，不公開編輯器、模型、專案或對話內容。

週報提供深淺色及手機版。圖片使用內容雜湊檔名，資料更新時一併更新 README，避免 GitHub 顯示舊快取；每個深淺色/尺寸版本保留最近八份圖片，讓快取中的舊 README 仍可載入圖片。

公開動態使用未登入的 GitHub public events API，只列兩筆，排除本個人主頁倉庫的自動更新；預設折疊。沒有資料時顯示真實空狀態。

贪吃蛇每天約北京時間 09:43 更新。Platane/snk 產生貢獻路徑，再由 scripts/smooth_snake.py 以每秒 240 像素的線性 CSS 位移（每格約 0.067 秒，參考 alongw 的 CSS 動畫實作）前進：吃掉第一塊綠色，在第二塊前停下變紅並露出白色叉叉眼；保持清晰兩秒後，整幅畫面（含蛇和叉叉眼）切換為固定模糊快照，同時顯示紅色的「蛇饿死了」。死亡階段約 1.5 秒，不使用淡入、上浮或動態模糊半徑。無進度條。尾節各有終點，停下時不會堆疊。尊重系統減少動態效果設定。
