...
贪吃蛇每天約北京時間 09:43 更新。Platane/snk 產生貢獻路徑，再由 scripts/smooth_snake.py 轉成十秒循環的劇情動畫：沿圓角路徑前進，停在第一塊綠色格子前，變紅並顯示「蛇饿死了」。尾節各有終點，停下時不會堆疊。綠色食物維持原樣，移除進度條。死亡階段淡入固定的高斯模糊快照；只切換透明度，不逐幀改變模糊半徑。尊重系統減少動態效果設定，該模式顯示靜態圖。
