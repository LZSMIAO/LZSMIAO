...
贪吃蛇每天約北京時間 09:43 更新。Platane/snk 產生貢獻路徑，再由 scripts/smooth_snake.py 以每秒 180 像素前進：吃掉第一塊綠色，在第二塊前停下變紅，整幅畫面（含蛇）切換為固定模糊快照，只顯示「蛇饿死了」。死亡階段約 1.5 秒，不使用淡入、上浮或動態模糊半徑。無進度條。尾節各有終點，停下時不會堆疊。尊重系統減少動態效果設定。
